...
\subsection{Ablauf beim Programmieren}
In diesem Abschnitt möchten wir ein paar Tipps zum Ablauf des Programmierens geben. Betrachten wir dazu einmal das Flowchart in Abbildung~\ref{fig:fehlersuche}. Jedes Programm sollte sorgfältig geplant werden: Wir müssen überlegen, was zu tun ist, und wie man dies geschickt und effizient im Algorithmus umsetzen kann. Nachdem wir den Algorithmus in C gecodet haben, kompilieren wir ihn. Wenn es hier Warnungen oder Fehler gibt, müssen wir sie zunächst verstehen und beheben. Dies wiederholen wir, bis das Programm fehlerfrei und warnungsfrei kompiliert wird. Als nächstes testen wir, ob das Programm das tut, was wir erwarten. Dazu benutzen wir in der einfachsten Form verschiedene Eingabeparameter und schauen, ob die Ausgabe des Programms richtig ist. Wenn dies nicht der Fall ist, kann es Fehler in unserer Anfangsüberlegung geben und wir müssen den Algorithmus anpassen. Eventuell ist aber auch der Algorithmus korrekt und der Fehler liegt im Code (trotz fehlerfreien Kompilierens). Haben wir das eine oder andere behoben, geht es entsprechend dem Flowchart von vorne los, bis wir mir relativ großer Sicherheit sagen können, dass das Programm tut was es soll und wir fertig sind\begin{footnote}{Die Fehlersuche im Code (das Debuggen) und das Testen des Codes (Unit-Tests, System-Tests) können durch professionelle Tools sehr erleichtert werden. Sobald ihr eine größeres Softwareprojekt startet, sollten diese auch unbedingt genutzt werden. Empfehlenswert ist dann auch die Versionskontrolle mit Hilfsmitteln wie z.B.\ git.}\end{footnote}.

\begin{figure}[htb]
\centering
\includegraphics[width=11.9cm]{flowchart.pdf}
\caption{Ablauf beim Programmieren.}
\label{fig:fehlersuche}
\end{figure}
...
Der gcc ist ein unter Linux entwickelter Compiler. Für eine ganze Sammlung von Linux-Programmen existieren Windows-Ports: Diese Sammlung heißt Cygwin. Wir werden hier kurz erläutern, wie Cygwin zu installieren und zu bedienen ist. 
